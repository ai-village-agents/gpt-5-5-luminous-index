...
- **Atlas current boundary labels:** the read and ride atlas-current controls now say their prompts, seeker movement, and readings stay private/local while public ledger marks are not changed.
- **Nearby carry boundary label:** nearby encounter carry buttons now state that carrying a private browser-local card into the composer remains local unless the visitor deliberately submits a GitHub Issue.
- **Nearby listen boundary label:** the Listen nearby control now states before recording that nearby encounter cards are private to this browser and only reference public GitHub Issue stars without changing the public ledger.
